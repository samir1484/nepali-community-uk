...
export async function generateMetadata({
  ...
  return buildLocationMetadata("VOLUNTEER", location);
}

export default async function VolunteerInLocationPage({
  params,
}: {
  params: Promise<{ location: string }>;
}) {
  const { location } = await params;
  return <LocationListingsPage type="VOLUNTEER" slug={location} />;
}
